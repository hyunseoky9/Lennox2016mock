function [receptacle] = perfinfosim(param)
%mxtjcor = 0;
%mxCcor = 0;
%mxBcor = 0;
...
for god = 1:godsim
  ...
  for i = 1:simtime
    ...
    if t_jmethod % max profitable to develop
      PVdiff = sum((x_rj(t_j:end)-x_fj(t_j:end))./edisc(t_j:end)) - ch; % present value difference since t_j
      ...
      PVdiff = sum((x_rj(t_j:end)-x_fj(t_j:end))./edisc(t_j:end)) - ch;
