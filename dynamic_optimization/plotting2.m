...
for pl = 1:length(what2pl)
  ...
  elseif what2pl(pl) == 11 % C histogram
    ...
    xlabel('C');
    title("C histogram");
  elseif what2pl(pl) == 12 % C histogram all contingent (to t_j)
    for j = tjv
      nexttile
      I = find(tjs == j);
  ...
  elseif what2pl(pl) == 13 % C histogram specific contingent
    nexttile
    val = 1;
    I = find(tjs == val);
    histogram(C); % used when plotting only distribution with certain t_j
    histogram(C(I)); % used when plotting only distribution with certain t_j
    Ctitle = '';
    Ctitle = strcat(Ctitle, sprintf('af=%.1f,ar=%.1f, tj%d',param(8),param(15),val));
    xlim([240,350]);
    %ylim([0 620]);
    %histogram(C);
    xlabel('C');
